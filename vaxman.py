...
Usage: python3 vaxman.py | python vaxman.py
       Requires pygame, install it with "pip install pygame" if the import
       fails.
       Run the program -> solve the pandemic -> profit.

Description: Vaxman is like Pacman but with a V.  In Vaxman, you need to
             collect all the yellow dots before the coronavirus ghosts take
             over your screen.  Every 30 seconds each corona-ghost will spilt
             itself in two, once the number ghostVID19s reaches 32 times their
             starting number (that'd be 128), the pacman race will be wiped
             out.  That's bad.  Fear not, no viral specter can survive being
             rammed with your uber covid-19 vaccination!
...
class Wall(pygame.sprite.Sprite):
    ...
    Instance Attributes:
        image <pygame.Surface>
            -- The visual representation of the wall.
        rect <pygame.Rect>
            -- Rectangular coordinates of the wall.  x and y positions can be
               changed by setting self.rect.left and self.rect.top.
    """
    ...
    def __init__(self, x: int, y: int, width: int, height: int, color: tuple):
        ...
        Parameters:
            ...
            color  -- Contains 3 int values representing R, G, and B.  The
                      image Surface will be filled with this color.
        """
...
class Block(pygame.sprite.Sprite):
    ...
    Instance Attributes:
        image <pygame.Surface>
            -- The visual representation of the block.
        rect <pygame.Rect>
            -- Rectangular coordinates of the block.  x and y positions can be
               changed by setting self.rect.x and self.rect.y.
    """
...
class Intersection(Block):
    ...
    Instance Attributes:
        directions <set: str>
            -- The paths that the intersection feeds into.
    """
    ...
    def __init__(self, width: int, directions: set, color=black):
        ...
        Keyword Arguments:
            color <tuple: int>
                -- Used for debugging, or if the background is a color other
                   than black.  (default (0,0,0)).
        """
...
class Collision(Exception):
    ...
    Instance Attributes:
        self.sprite <pygame.sprite.Sprite>
            -- The sprite that collided with a group of sprites.
        self.collided_with <list>
            -- The group of sprites collided with.
    """
    ...
    def __init__(self, sprites: list, *args: object) -> None:
        ...
        Parameters:
            sprites -- The list of sprites involved in the collision.  The
                       second element is a list of pygame Sprites that
                       collided with the first element.
            args -- Standard arguments to be fed into an exception (optional).
        """
...
class Player(pygame.sprite.Sprite):
    ...
    Instance Attributes:
        ...
        rect <pygame.Rect>
            -- Rectangular coordinates of the player.  x and y positions can be
               changed by setting self.rect.left and self.rect.top.
        prev_x <int>
            -- The x coordinate from before an update.
        prev_y <int>
            -- The y coordinate from before an update.

    Class Attributes
        change_x <int>
            -- Initial x speed vector.
        change_y <int>
            -- Initial y speed vector.
    """
...
class Ghost(Player):
    ...
    Instance Attributes:
        ...
        current_dir <str>
            -- The direction this ghost is currently moving in.

    Class Attributes:
        speed <int>
            -- The magnitude that speed should be changed by for Ghosts.
    """
    ...
    def __init__(self, x: int, y: int, filename: str, groups):
        ...
        Parameters:
            (See the documentation of the parent class (Player))
        """
    ...
    def choose_dir(self, prev_dirs, all_dirs) -> str:
        """
        Randomly choose a new direction that has not been previously taken.

        Parameters:
            prev_dirs <set: str>
                -- The set of previous direction strings.
            all_dirs <set: str>
                -- The set of all possible direction strings.

        Side Effects:
            -- Set prev_dirs to the empty set if it contains all directions.
    ...
    def __mul__(self, num: int) -> list:
        """
        Create duplicates of the Ghost instance.

        Always returns this instance as long as num is at least 1.  Returns an
        empty list if num is less than 1.

        Parameters:
            num -- The number of Ghosts to return.  Will make duplicates if
                   greater than 1.

        Side Effects:
            -- Automatically adds the new ghosts to the same pygame sprite
               groups as the original.

        Return this Ghost and its duplicates.
        """
